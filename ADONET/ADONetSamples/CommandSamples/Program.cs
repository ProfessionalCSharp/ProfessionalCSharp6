...
namespace CommandSamples
{
    class Program
    {
            ...
            using (var connection = new SqlConnection(GetConnectionString()))
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        WriteLine($"{recursionLevel} {businessEntityId} {firstName} {lastName}");
                    }
                }
            }
        }
        ...
        public static void ShowUsage()
        {
            ...
            WriteLine("\t-nq\tExecute Non-Query");
            WriteLine("\t-s\tExecute Scalar");
            WriteLine("\t-r\tExecute Reader");
            WriteLine("\t-sp\tStored Procedure");
        }
            ...
            using (var connection = new SqlConnection(GetConnectionString()))
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var s = reader[2];
                        WriteLine(s);
                        //WriteLine($"{reader.GetString(1)} {reader.GetString(2) ?? String.Empty}");
                    }
                }
            }
