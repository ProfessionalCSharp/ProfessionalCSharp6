ReadMe - Code Samples for Chapter 9, Delegates, Lamdas, and Events

The samples are available in two variants:
a) using the full .NET Framework
b) using .NET Core

The sample code for the full framework is available in the directory Net46DelegatesAndEventsSamples. The source code is not directly available in the Net46DelegatesAndEventsSamples directory, but is linked instead. The C# source code files are the same as used with the .NET Core samples. 
You can open the solution file Net46DelegatesAndEventsSamples.sln with Visual Studio and work with it.

The sample code for .NET Core is available in the directory DelegatesAndEventsSamples. To use these source files with Visual Studio, you need to install ASP.NET 5 tools in addition to Visual Studio 2015.
